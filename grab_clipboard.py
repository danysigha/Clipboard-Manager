...
class ClipboardManager:
    ...
    def updateUI(self):
        ...
        if self._currentCount != self._pb.changeCount():
            ...
            # self._cardRenderer.numCheck = 1  # go back to all cards before you add item - does not work
            ...
            if NSStringPboardType in data_type:
                ...
                if validators.url(pbstring):
                    # pbstring = "<a href={}>{}</a>".format(pbstring, pbstring)
                    category = "URL"
                else:
                    category = "Text"
                ...
                # self._cardRenderer.initializeUI(self._cardRenderer._dao.getAllCards(), self._cardRenderer._position)

                card_generator.CardObject(self._cardRenderer.parent, self._cardRenderer._position,self._dao,
                                              self._cardRenderer).addToInterface(card_id, pbstring, category,
                ...
                #                                                1, 0)
                card_generator.CardObject(self._cardRenderer.parent, self._cardRenderer._position, self._dao,
                                              self._cardRenderer).addToInterface(card_id, filepath, category,
                                                               datetime.datetime.now(), datetime.datetime.now(), 1, 0, 0)
