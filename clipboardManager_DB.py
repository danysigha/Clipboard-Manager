...
# remove cardModifiedDate, folderID
...
# remove currentCardID, defaultFolderID
...
def pasteCard(cardID): # needs to go
    conn = sqlite3.connect('ClipboardManager_DB.db, pragma key=’secretKey’')
    cursor = conn.cursor()
...
def automaticDelete_shelftime(userID): # must be called in login page
    conn = sqlite3.connect('ClipboardManager_DB.db, pragma key=’secretKey’')
    cursor = conn.cursor()
...
# sorting
def sortCard(folderID, sortType): # needs to go
    conn = sqlite3.connect('ClipboardManager_DB.db, pragma key=’secretKey’')
    cursor = conn.cursor()
...
# create folder (HERE)
def createFolder(userID, folderName): # needs to go
    conn = sqlite3.connect('ClipboardManager_DB.db, pragma key=’secretKey’')
    cursor = conn.cursor()
...
# delete folder
def deleteFolder(userID, folderID): # needs to go
    conn = sqlite3.connect('ClipboardManager_DB.db, pragma key=’secretKey’')
    cursor = conn.cursor()
    ...
    conn.close()
# update folder
def updateFolderID_all(userID, currentFolderID, newFolderID): # needs to go
    conn = sqlite3.connect('ClipboardManager_DB.db, pragma key=’secretKey’')
    cursor = conn.cursor()
...
def updateFolderID_indiv(userID, cardID, newFolderID): # needs to go
    conn = sqlite3.connect('ClipboardManager_DB.db, pragma key=’secretKey’')
    cursor = conn.cursor()
